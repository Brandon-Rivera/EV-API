...
{
    const body = req.body; 
    const sql = `INSERT INTO questions(id,questionType,questionDescription,isActive)VALUES(?, ?, ?, ?)`;
    conexion.query(sql, [body.id, body.questionType, body.questionDescription, body.isActive], (error, results, fields) =>{
        if(error){
            res.send(error);
        }
        res.json(results);
