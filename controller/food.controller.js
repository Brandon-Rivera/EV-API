...
{
    const body = req.body; 
    const sql = `INSERT INTO food(id,foodName,foodDesc,lipidos,carbohidratos,proteinas,quantity,stock,expiration)VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)`;
    conexion.query(sql, [body.id, body.foodName, body.foodDesc, body.lipidos, body.carbohidratos, body.proteinas, body.quantity,body.stock,body.expiration], (error, results, fields) =>{
        if(error){
            res.send(error);
        }
        res.json(results);
